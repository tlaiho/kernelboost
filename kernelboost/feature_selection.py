...
class JMISelector(FeatureSelector):
    """
    DEFAULT SELECTOR. Feature selection by joint mutual information (JMI, Yang 
    & Moody 1999). Uses the mean joint MI I((Xs, Xj); y) to measure relevance,
    redundancy and synergy with the same metric. Kernel sizes progress from small 
    to large.
...
class SmartSelector(FeatureSelector):
    """
    DEPRECATED, IN USE FOR BACKWARD COMPATIBILITY ONLY.
